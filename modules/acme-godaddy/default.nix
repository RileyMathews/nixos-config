...
in
{
  options.myAcme = {
    enable = mkOption {
      type = types.bool;
      default = false;
      description = "Whether to enable the custom ACME configuration with GoDaddy DNS.";
    };
  ...
  config = mkIf cfg.enable {
    age.secrets.godaddy-credentials = {
      file = ../../secrets/godaddy-credentials.age;
      mode = "0400";
      owner = "acme";
      group = "acme";
    };
    ...
    security.acme = {
      ...
      certs = mapAttrs (hostname: certCfg: {
        dnsProvider = "godaddy";
        group = certCfg.group;
        environmentFile = config.age.secrets.godaddy-credentials.path;
      }) cfg.certs;
    };
  };
}
